...
//    GtkIMContext* im_context;
      ...
      Qt::CursorShape active_cursor_type = Qt::ArrowCursor;
   };
...
//  : im_context(gtk_im_context_simple_new())
   ...
   namespace
   {
      ...
      Qt::CursorShape view_cursor_type = Qt::ArrowCursor;
...
/*
   static void change_window_cursor(QWidget* widget, Qt::CursorShape type)
   {
   ...
   bool on_event_crossing(GtkWidget* widget, GdkEventCrossing* event, gpointer user_data)
   {
      auto& base_view = get(user_data);
      auto* host_view_h = platform_access::get_host_view(base_view);
   ...
   bool on_key(GtkWidget* widget, GdkEventKey* event, gpointer user_data)
   {
      auto& base_view = get(user_data);
      auto* host_view_h = platform_access::get_host_view(base_view);
...
*/
   QWidget* make_view(base_view& view, QWidget* parent)
   {
      auto* content_view = new QWidget(parent);
/*
